...
describe AsciicrystalDiagram::Cache do
  ...
  describe ".hash_key" do
    it "returns a hex SHA256 digest" do
      key = AsciicrystalDiagram::Cache.hash_key("hello")
    ...
    it "returns the same hash for the same input" do
      a = AsciicrystalDiagram::Cache.hash_key("test source")
      b = AsciicrystalDiagram::Cache.hash_key("test source")
      a.should eq(b)
    end

    it "returns different hashes for different inputs" do
      a = AsciicrystalDiagram::Cache.hash_key("source A")
      b = AsciicrystalDiagram::Cache.hash_key("source B")
      a.should_not eq(b)
    end
  end
  ...
  describe "#cache_path" do
    it "constructs path from key and format" do
      cache = AsciicrystalDiagram::Cache.new(cache_dir)
      key = AsciicrystalDiagram::Cache.hash_key("src")
